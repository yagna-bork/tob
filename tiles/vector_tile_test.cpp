...
void test_edges_to_string() {
  ...
  std::cout << "test_edges_to_string(): " << edges_to_string(building)
            << std::endl;
}
...
void test_coord_converter() {
  std::vector<FPoint> bng_points{
      {580045.0, 183233.0}, {580049.0, 183276.0},  {580058.0, 183279.0},
      {580070.0, 183236.0}, {580018.0, 183235.0},  {580012.0, 183258.0},
      {580012.0, 183258.0}, {580018.0, 183275.0},  {580012.0, 183268.0},
      {580009.0, 183262.0}, {580009.0, 183262.0},  {580067.0, 183282.0},
      {580019.0, 183227.0}, {580008.75, 18326.15}, {580032.0, 183292.0},
      {580032.0, 183292.0}, {580032.0, 183292.0},  {580032.0, 183292.0},
      {580032.0, 183292.0}, {580032.0, 183292.0},  {580083.0, 183238.0},
      {580023.0, 183217.0}, {580007.0, 183283.0},  {580007.0, 183283.0},
...
void test_get_tile_rows_cols() {
  ...
  CoordConverter cc(centre);
  std::vector<Point> cell_coords = {{502, -1014}, {-256, -256}, {256, -256},
                                    {786, -256},  {-256, 256},  {1014, 10},
                                    {1524, 502},  {-256, 768},  {10, 1014}};
  std::vector<std::pair<int, int>> expected = {
      {21301, 14613}, {21302, 14612}, {21302, 14613},
      {21302, 14614}, {21303, 14612}, {21303, 14614},
      {21303, 14615}, {21304, 14612}, {21304, 14613}};
  std::vector<std::pair<int, int>> res;
  std::transform(cell_coords.begin(), cell_coords.end(),
                 std::back_inserter(res),
                 [&cc](const Point &p) { return cc.get_tile_row_col(p); });
  bool test_passed = std::equal(
      res.begin(), res.end(), expected.begin(),
  ...
  if (test_passed) {
    ...
    std::cout << diff.first->first << "," << diff.first->second << " "
              << diff.second->first << "," << diff.second->second << std::endl;
  }
}
...
void test_get_combined_tile() {
  ...
  Tile tile = get_combined_tile(CURL_HANDLE, grid_positions,
                                /*centre_row=*/21303, /*centre_col=*/14613);
  std::ofstream output(COMBINATION_TEST_TILE_PATH,
...
void test_get_enclosure_type() {
  ...
  } else if (get_enclosure_type({3, 3}, shape, pen_mp) !=
             EnclosureType::OUTSIDE) {
