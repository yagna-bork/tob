...
public:
  ...
  int get_centre_row() const { return tile_row; }

  int get_centre_col() const { return tile_col; }
...
public:
  ...
  Tile tile(const GridPos &pos) {
    ...
    if (sqlite3_step(stmt) != SQLITE_ROW) {
      ...
      const char *data =
          static_cast<const char *>(sqlite3_column_blob(stmt, 0));
...
int get_tiles_api_url(char url_buff[], size_t buff_sz, int grid_row,
                      int grid_col) {
  init_config();
