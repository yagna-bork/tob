...
bool FileStat::is_file() const { return ret_ >= 0 && S_ISREG(st_.st_mode); }
bool FileStat::is_dir() const { return ret_ >= 0 && S_ISDIR(st_.st_mode); }
...
template <typename T> bool parse_header(const char *beg, const char *end, T fn);
...
template <typename T>
bool process_server_socket(const std::atomic<socket_t> &svr_sock, socket_t sock,
...
bool get_ip_and_port(const struct sockaddr_storage &addr, socklen_t addr_len,
...
bool nocompressor::compress(const char *data, size_t data_length, bool /*last*/,
                            Callback callback) {
  if (!data_length) {
    return true;
  }
...
bool gzip_compressor::compress(const char *data, size_t data_length, bool last,
                               Callback callback) {
  assert(is_valid_);
...
bool zstd_compressor::compress(const char *data, size_t data_length, bool last,
                               Callback callback) {
  std::array<char, CPPHTTPLIB_COMPRESSION_BUFSIZ> buff{};
...
ReadContentResult read_content_without_length(Stream &strm,
...
template <typename T>
bool write_content_without_length(Stream &strm,
                                  const ContentProvider &content_provider,
...
template <typename T>
bool redirect(T &cli, Request &req, Response &res, const std::string &path,
...
std::string serialize_multipart_formdata(const UploadFormDataItems &items,
                                         const std::string &boundary,
                                         bool finish = true) {
  std::string body;
...
std::pair<size_t, size_t> get_range_offset_and_length(Range r,
...
std::string Request::get_header_value(const std::string &key, const char *def,
                                      size_t id) const {
...
FormData MultipartFormData::get_file(const std::string &key, size_t id) const {
...
std::string Response::get_header_value(const std::string &key, const char *def,
                                       size_t id) const {
  return detail::get_header_value(headers, key, def, id);
}
...
std::string make_host_and_port_string_always_port(const std::string &host,
                                                  int port) {
...
Server &Server::set_error_handler_core(Handler handler, std::false_type) {
...
Server &Server::set_trusted_proxies(const std::vector<std::string> &proxies) {
...
bool Server::write_response(Stream &strm, bool close_connection, Request &req,
                            Response &res) {
...
bool Server::write_response_with_content(Stream &strm, bool close_connection,
...
bool Server::write_content_with_provider(Stream &strm, const Request &req,
                                         Response &res,
                                         const std::string &boundary,
                                         const std::string &content_type) {
...
socket_t Server::create_server_socket(const std::string &host, int port,
                                      int socket_flags,
...
void Server::output_error_log(const Error &err, const Request *req) const {
  if (error_logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
...
bool ClientImpl::handle_request(Stream &strm, Request &req, Response &res,
...
bool ClientImpl::create_redirect_client(const std::string &scheme,
...
bool ClientImpl::write_content_with_provider(Stream &strm, const Request &req,
                                             Error &error) const {
...
std::unique_ptr<Response> ClientImpl::send_with_content_provider_and_receiver(
...
void ClientImpl::output_log(const Request &req, const Response &res) const {
  if (logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
    logger_(req, res);
  }
...
void ClientImpl::output_error_log(const Error &err, const Request *req) const {
  if (error_logger_) {
    std::lock_guard<std::mutex> guard(logger_mutex_);
    error_logger_(err, req);
  }
...
bool ClientImpl::process_request(Stream &strm, Request &req, Response &res,
                                 bool close_connection, Error &error) {
  // Send request
...
Result ClientImpl::Get(const std::string &path, DownloadProgress progress) {
...
Result ClientImpl::Patch(const std::string &path, const std::string &body,
                         const std::string &content_type,
                         UploadProgress progress) {
...
Result ClientImpl::Delete(const std::string &path, DownloadProgress progress) {
...
Result ClientImpl::Delete(const std::string &path, const std::string &body,
                          const std::string &content_type,
                          DownloadProgress progress) {
...
void ClientImpl::set_max_timeout(time_t msec) { max_timeout_msec_ = msec; }
...
template <typename T>
bool process_server_socket_ssl(const std::atomic<socket_t> &svr_sock, SSL *ssl,
                               socket_t sock, size_t keep_alive_max_count,
                               time_t keep_alive_timeout_sec,
                               time_t read_timeout_sec,
                               time_t read_timeout_usec,
...
bool SSLSocketStream::is_readable() const { return SSL_pending(ssl_) > 0; }
...
SSLClient::SSLClient(const std::string &host, int port, X509 *client_cert,
                     EVP_PKEY *client_key,
                     const std::string &private_key_password)
...
void SSLClient::load_ca_cert_store(const char *ca_cert, std::size_t size) {
...
Result Client::Get(const std::string &path, ResponseHandler response_handler,
                   ContentReceiver content_receiver,
                   DownloadProgress progress) {
...
Result Client::Post(const std::string &path, const UploadFormDataItems &items,
                    UploadProgress progress) {
...
Result Client::Put(const std::string &path, const UploadFormDataItems &items,
                   UploadProgress progress) {
...
Result Client::Patch(const std::string &path, const UploadFormDataItems &items,
                     UploadProgress progress) {
...
Result Client::Delete(const std::string &path, DownloadProgress progress) {
...
void Client::set_logger(Logger logger) { cli_->set_logger(std::move(logger)); }
